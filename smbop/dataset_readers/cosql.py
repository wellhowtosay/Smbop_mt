...
@DatasetReader.register("smbop_cosql")
class SmbopCoSqlDatasetReader(DatasetReader):
    def __init__(
            self,
            lazy: bool = True,
        ...
        self.cache = TensorCache(cache_directory)  # 解析dataset后存储到文件中
    ...
    def _read_examples_file(self, file_path: str):
        ...
        cont_flag = True
        sent_set = set()
        for ins_cnt, ins in self.cache:
            if cnt >= self._max_instances:
                break
            if ins is not None:
                yield ins
                cnt += 1
            sent_set.add(ins_cnt)
        ...
        if cont_flag:
            with open(file_path, "r") as data_file:
                ...
                ins_cnt = 0
                for dialog_id, ex in enumerate(json_obj):
                    his_utt = []
                    his_sql = []
                    ...
                    for turn in ex["interaction"]:
                        ...
                        if len(cache_buffer) > 50:
                            ...
                            cache_buffer = []
                        if ins_cnt in sent_set:  # cached instance id的集合
                            ins_cnt += 1
                            continue
                        else:
                            ins = self.create_instance(turn, db_id, his_utt, his_sql, dialog_id)
                            cache_buffer.append([ins_cnt, ins])
                            ins_cnt += 1
                        if ins is not None:
                            yield ins
                            cnt += 1
    ...
    def create_instance(self, turn, db_id, his_utt, his_sql, dialog_id):
        sql = None
        sql_with_values = None
        query_toks = turn["query"].split(" ")
        if "query_toks" in turn:
            try:
                turn["query_toks_no_value"] = disamb_sql.fix_number_value(query_toks, turn["query_toks_no_value"])
                sql = disamb_sql.disambiguate_items(
                    db_id,
                    turn["query_toks_no_value"],
                    self._tables_file,
                    allow_aliases=False,
                )
                sql_with_values = disamb_sql.sanitize(turn["query"])
            except Exception as e:
                # there are two examples in the train set that are wrongly formatted, skip them
                print(f"error with {turn['query']}", db_id)
                return None

        ins = self.text_to_instance(
            utterance=turn["utterance"] + " [SEP] " + " ".join(his_utt),
            db_id=db_id,
            sql=sql,
            sql_with_values=sql_with_values,
            dialog_id=dialog_id
        )
        his_utt.append(turn["utterance"])
        his_sql.append(sql_with_values)
        return ins

    def text_to_instance(
            self, utterance: str, db_id: str, sql=None, sql_with_values=None, dialog_id: int = None
    ):
        fields: Dict[str, Field] = {
            "db_id": MetadataField(db_id),
            "dialog_id": MetadataField(dialog_id),
        }
